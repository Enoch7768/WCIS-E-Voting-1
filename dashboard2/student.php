...
async function openAssignmentDetails(asmId) {
    ...
    if(!data.error) {
        ...
        container.innerHTML = `
            <div><strong>PACE:</strong> ${esc(data.pace)}</div>
            <div><strong>Assigned Supervisory Teacher:</strong> ${esc(data.teacher_name)} (${esc(data.teacher_email)})</div>
            <div><strong>Milestone Deadline Cap:</strong> ${esc(data.due_date)}</div>
            <div><strong>Status:</strong> <span class="badge">${esc(data.status)}</span></div>
        `;
...
async function openChatModal(teacherId, teacherName) {
    ...
    logs.forEach(m => {
        ...
        let isMe = m.sender_id == <?= (int)$student_id ?>;
        let attachmentHtml = '';
        if (m.attachment_path) {
            ...
            if (['jpg','jpeg','png','gif','webp'].includes(ext)) {
                ...
                attachmentHtml = `<div><a href="${safePath}" target="_blank" rel="noopener noreferrer" class="btn btn--ghost">📎 ${fileName}</a></div>`;
            }
        }
